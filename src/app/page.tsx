    ...
    <main className="min-h-screen bg-slate-950">
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-5 py-14 lg:grid-cols-[1.05fr_.95fr] lg:py-20">
        <div className="flex flex-col justify-center">
          <p className="text-sm font-semibold uppercase tracking-[0.28em] text-cyan-200">Aviation cargo partner network</p>
        ...
        <div className="rounded-lg border border-white/10 bg-[#0A1F44] p-4 shadow-2xl shadow-cyan-950/30">
          <div className="rounded-md bg-slate-950/70 p-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div>
                <p className="text-xs uppercase tracking-[0.22em] text-cyan-200">Live cargo desk</p>
                <h2 className="mt-1 text-xl font-semibold text-white">Tender control tower</h2>
              </div>
              <Radar className="h-8 w-8 text-cyan-300" />
            </div>
            <div className="mt-4 space-y-3">
              {tenders.slice(0, 2).map((tender) => (
                <Card key={tender.id} className="bg-white text-slate-950">
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="font-semibold">{tender.title}</p>
                        <p className="mt-1 text-sm text-slate-500">{tender.lanes}</p>
                      </div>
                      <span className="rounded-full bg-cyan-50 px-2 py-1 text-xs font-semibold text-cyan-700">{tender.deadline}</span>
                    </div>
                  </CardContent>
                </Card>
              ))}
