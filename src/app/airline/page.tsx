...
export default function AirlineDashboardPage() {
  ...
  const gsaMap: Record<string, { name: string; color: string; flights: number; tonnage: number; revenue: number; lfSum: number; yieldSum: number }> = {};
  for (const f of trackedFlights) {
    if (!gsaMap[f.gsaName]) {
      gsaMap[f.gsaName] = { name: f.gsaName, color: f.gsaColor, flights: 0, tonnage: 0, revenue: 0, lfSum: 0, yieldSum: 0 };
    }
  ...
  // --- Top routes ---
  const topRoutes = [...trackedFlights]
    .sort((a, b) => b.revenue - a.revenue)
    .slice(0, 5);
  ...
  return (
    <>
      <Topbar title="Sales overview" subtitle="AeroBridge Cargo" />
      <main className="space-y-6 p-5">
        <FlightWorldMap
          title="Live flight tracker"
          subtitle="Active flights connected to AeroBridge Cargo sales teams and GSA partners."
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_1.4fr]">
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-ink-muted">Revenue split between direct airline sales and GSA-sold capacity</p>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <div className="mt-2 flex items-center gap-4 text-xs text-ink-muted">
                  <span className="flex items-center gap-1.5"><span className="inline-block h-2 w-2 rounded-full bg-brand" />Airline direct</span>
                  <span className="flex items-center gap-1.5"><span className="inline-block h-2 w-2 rounded-full bg-cyan-accent" />GSA sold</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-border-ui pt-4">
                ...
                <div className="text-right">
                  <p className="text-xs text-ink-muted">Total tonnage</p>
          ...
          <Card>
            <CardHeader className="flex-row items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-sm text-ink-muted">Ranked by revenue, current active flights</p>
              </div>
                    ...
                    <div className="mt-1.5 flex items-center gap-2">
                      <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-surface2">
                        <div
                          ...
                          style={{ width: `${(flight.revenue / maxRouteRevenue) * 100}%`, backgroundColor: flight.airlineColor }}
                        />
                      </div>
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm text-ink-muted">Revenue, tonnage and load factor contribution per GSA partner</p>
            </div>
...
function ChannelRow({ label, color, flights, revenue, percent }: {
  label: string; color: string; flights: number; revenue: number; percent: number;
}) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl border border-border-ui bg-surface2 px-4 py-3">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-[11px] text-ink-muted">{flights} {flights === 1 ? "flight" : "flights"}</p>
        </div>
      </div>
      <div className="text-right">
...
function LfBadge({ value }: { value: number }) {
  const color =
    ...
    value >= 70 ? "text-amber-600 bg-amber-50 dark:bg-amber-900/20 dark:text-amber-400" :
