...
import { AlertTriangle, CheckCircle2, Clock, Inbox, Plus, Send, ShieldCheck, X } from "lucide-react";
...
import type { MandateQuote, MandateQuoteStatus } from "@/lib/services/mandate-execution-store";
...
export default function QuotesPage() {
  ...
  async function refresh() {
    ...
    const [contractData, quoteData] = await Promise.all([contractRes.json(), quoteRes.json()]);
    ...
    <>
      ...
      <main className="space-y-5 p-5">
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard icon={<Clock className="h-4 w-4" />} label="SLA target" value={`${selectedContract?.controlRules?.quoteResponseSlaHours ?? 4}h`} />
        </section>
        ...
        <div className="space-y-3">
          {quotes.length === 0 ? (
            ...
            <QuoteCard key={quote.id} quote={quote} onCounter={() => actionQuote(quote, "counter")} onDecline={() => actionQuote(quote, "decline")} />
          ))}
        </div>
      </main>
    </>
...
function QuoteCard({ quote, onCounter, onDecline }: { quote: MandateQuote; onCounter: () => void; onDecline: () => void }) {
